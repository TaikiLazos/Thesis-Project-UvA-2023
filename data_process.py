...
def load_x_y(path : str, n: int, normalize:bool = False):
    ...
    x = [window.to_list() for window in data['Adj Close'].rolling(window=n)][n - 1:-n]
    y = [window.to_list() for window in data['Adj Close'].rolling(window=n)][2*n - 1:]
